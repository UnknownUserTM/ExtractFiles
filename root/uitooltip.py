...
IS_NEW_RING_2020 = [71143, 71149, 71135, 160498, 71145, 160499, 71136, 160500, 160200]
...
class ToolTip(ui.ThinBoard):
	...
	def AppendStatusTextLine(self, text, onLineStatus, color = FONT_COLOR):
		textLine = ui.TextLine()
		textLine.SetParent(self)
		textLine.SetFontName(self.defFontName)
		textLine.SetPackedFontColor(color)
		...
		if onLineStatus == 1:
			textLineCount.SetPackedFontColor(self.POSITIVE_COLOR)
			textLineCount.SetText("Aktiv")
		else:
			textLineCount.SetPackedFontColor(self.NEGATIVE_COLOR)
			textLineCount.SetText("Inaktiv")
...
class ItemToolTip(ToolTip):
	...
	def AddItemData(self, itemVnum, metinSlot, attrSlot = 0, flags = 0, unbindTime = 0, stackCount = 0, preview = 1):
		self.itemVnum = itemVnum
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ #
		...
		elif itemVnum in IS_NEW_RING_2020:
			for i in xrange(5):
	...
	if app.RENDER_TARGED:
		def __ModelPreview(self, Vnum, test, model):
			if constInfo.DISABLE_MODEL_PREVIEW == 1: #konnt ihr gern einbauen in constinfo.py
				return
			RENDER_TARGET_INDEX = 1
			if constInfo.refinegui != 1: # Refine offen Pos anders
				...
				self.ModelPreviewText.SetText("Item Vorschau")
