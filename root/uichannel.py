...
class ChannelBoard(ui.ScriptWindow):
	def __init__(self):
	...
	def __MakeBoard(self):
		...
		self.ServerName.SetText(str(GFHhg54GHGhh45GHGH.GetServerInfo()).split(",")[0])
	...
	def __GetServerID(self):
		...
		for i in serverInfo.REGION_DICT[regionID].keys():
			if serverInfo.REGION_DICT[regionID][i]["name"] == GFHhg54GHGhh45GHGH.GetServerInfo().split(",")[0]:
				serverID = int(i)
				break
	...
	def __OnClickConnectButton(self):
		...
		if not channelID:
			return
		if GFHhg54GHGhh45GHGH.GetServerInfo().strip().split(", ")[1] == self.ChannelList.textDict[self.ChannelList.selectedLine].strip().split(" ")[0]:
			...
			return
		GFHhg54GHGhh45GHGH.SetServerInfo(GFHhg54GHGhh45GHGH.GetServerInfo()[:-1] + str(channelID))
		self.Close()
		GFHhg54GHGhh45GHGH.SendChatPacket("/channel "+str(channelID))

	def DirectConnect(self, ChannelIP, ChannelPort, AuthServerIP, AuthServerPort):
		GFHhg54GHGhh45GHGH.SetLoginInfo(decode_string(GFHhg54GHGhh45GHGH.ACCOUNT_ID), decode_string(GFHhg54GHGhh45GHGH.ACCOUNT_PW))
		GFHhg54GHGhh45GHGH.ConnectToAccountServer(ChannelIP, ChannelPort, AuthServerIP, AuthServerPort)
		GFHhg54GHGhh45GHGH.DirectEnter(0)
		GFHhg54GHGhh45GHGH.SendSelectCharacterPacket(0)
