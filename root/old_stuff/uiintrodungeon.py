...
class IntroDungeonBoard(ui.ScriptWindow):
	TextLines = {}
	...
	def SetPage(self):
		self.ClearTextLines()
		TextLines = settinginfo.IntroDungeonTextLines
		if TextLines[self.TextBoxIndex][self.PageIndex][0][1] == "head_img":
			self.HeadImages.LoadImage("intro_dungeon_images/"+str(TextLines[self.TextBoxIndex][self.PageIndex][0][0])+".tga")
	...
	def GAME_InitIntroDungeonGUI(self,textBlockIndex):
		self.TextBoxIndex = textBlockIndex
		self.PageIndex = 0
		self.SetPage()
		self.Board.Show()		

	def GAME_SetQuestIndex(self,questIndex):
		self.curQuestIndex = int(questIndex)
