...
class WhisperDialog(ui.ScriptWindow):
	...
	def AddFrind(self):
		GFHhg54GHGhh45GHGH.SendMessengerAddByNamePacket(self.targetName)
	...
	def ReportViolentWhisper(self):
		GFHhg54GHGhh45GHGH.SendChatPacket("/reportviolentwhisper " + self.targetName)

	def IgnoreTarget(self):
		GFHhg54GHGhh45GHGH.SendChatPacket("/ignore " + self.targetName)
	...
	def SendWhisper(self):
		...
		if textLength > 0:
			if GFHhg54GHGhh45GHGH.IsInsultIn(text):
				chat.AppendChat(chat.CHAT_TYPE_INFO, localeInfo.CHAT_INSULT_STRING)
				return

			GFHhg54GHGhh45GHGH.SendWhisperPacket(self.targetName, text)
			self.chatLine.SetText("")

			chat.AppendWhisper(chat.WHISPER_TYPE_CHAT, self.targetName, fgGHGjjFHJghjfFG1545gGG.GetName() + " : " + text)
