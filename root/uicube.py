...
class CubeWindow(ui.ScriptWindow):
	...
	def __OnSelectMaterialSlot(self, trash, resultIndex, materialIndex):
		...
		for itemVnum, itemCount in materialInfo[materialIndex]:
			...
			if fgGHGjjFHJghjfFG1545gGG.GetItemCountByVnum(itemVnum) >= itemCount:
				for i in xrange(fgGHGjjFHJghjfFG1545gGG.INVENTORY_SLOT_COUNT):
					vnum = fgGHGjjFHJghjfFG1545gGG.GetItemIndex(i)
					count= fgGHGjjFHJghjfFG1545gGG.GetItemCount(i)
						...
						for slotPos in xrange(self.cubeSlot.GetSlotCount()):
							...
							if not slotPos in self.cubeItemInfo:
								...
								if cubeColumn in self.cubeItemInfo:
									columnVNUM = fgGHGjjFHJghjfFG1545gGG.GetItemIndex(self.cubeItemInfo[cubeColumn])
								...
								if 0 < currentSlotLine and slotPos - self.CUBE_SLOT_COUNTX in self.cubeItemInfo:
									upperColumnVNUM = fgGHGjjFHJghjfFG1545gGG.GetItemIndex(self.cubeItemInfo[slotPos - self.CUBE_SLOT_COUNTX])
								...
								if True == bCanAddSlot:
									...
									GFHhg54GHGhh45GHGH.SendChatPacket("/cube add %d %d" % (slotPos, i))
	...
	def __OverInMaterialSlot(self, trash, resultIndex, col):
		...
		for itemVnum, count in materialInfo[col]:
			item.SelectItem(itemVnum)
			if fgGHGjjFHJghjfFG1545gGG.GetItemCountByVnum(itemVnum) >= count:
	...
	def Open(self):
		...
		self.isUsable	= True
		(self.xShopStart, self.yShopStart, z) = fgGHGjjFHJghjfFG1545gGG.GetMainCharacterPosition()
	...
	def __OnCloseButtonClick(self):
		if self.isUsable:
			...
			print "큐브 닫기"
			GFHhg54GHGhh45GHGH.SendChatPacket("/cube close")
	...
	def __OnAcceptButtonClick(self):
		...
		#	GFHhg54GHGhh45GHGH.SendChatPacket("/cube add " + str(invenPos))
		GFHhg54GHGhh45GHGH.SendChatPacket("/cube make")			
	...
	def __OnSelectItemSlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			GFHhg54GHGhh45GHGH.SendChatPacket("/cube del %d " % selectedSlotPos)
