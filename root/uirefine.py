...
class RefineDialog(ui.ScriptWindow):
	...
	def GetRefineSuccessPercentage(self, scrollSlotIndex, itemSlotIndex):
		...
		if -1 != scrollSlotIndex:
			if fgGHGjjFHJghjfFG1545gGG.IsRefineGradeScroll(scrollSlotIndex):
				curGrade = fgGHGjjFHJghjfFG1545gGG.GetItemGrade(itemSlotIndex)
		...
		for i in xrange(fgGHGjjFHJghjfFG1545gGG.METIN_SOCKET_MAX_NUM+1):
			if 0 == fgGHGjjFHJghjfFG1545gGG.GetItemMetinSocket(itemSlotIndex, i):
				break
	...
	def Accept(self):
		GFHhg54GHGhh45GHGH.SendItemUseToItemPacket(self.scrollItemPos, self.targetItemPos)
		self.Close()
...
class RefineDialogNew(ui.ScriptWindow):
	...
	def UpdateDialog(self):
		...
		self.board.SetSize(newWidth+20, newHeight)
		self.board.SetPosition(0, 50)
		self.toolTip.SetPosition(15 + 35 + 10, 38+20)
		self.titleBar.SetWidth(newWidth-15+30+20)
		self.titleBar.SetPosition(-8,7)
		self.SetSize(newWidth+20, newHeight+50)
