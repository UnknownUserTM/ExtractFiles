...
GENDER_NAME = {
	MAN : localeInfo.CREATE_GENDER_MALE,
	WOMAN : localeInfo.CREATE_GENDER_FEMALE,
}

SHAPE_NAME = {
	SHAPE0 : localeInfo.CREATE_SHAPE_NORMAL,
	SHAPE1 : localeInfo.CREATE_SHAPE_ALTERNATIVE,
}
...
class CreateCharacterWindow(ui.Window):
	...
	class CharacterRenderer(ui.Window):
		def OnRender(self):
			...
			app.SetCenterPosition(0.0, 0.0, 13.0)
	...
	def __init__(self, stream):
		...
		ui.Window.__init__(self)
		GFHhg54GHGhh45GHGH.SetPhaseWindow(GFHhg54GHGhh45GHGH.PHASE_WINDOW_CREATE, self)
	...
	def __del__(self):
		...
		GFHhg54GHGhh45GHGH.SetPhaseWindow(GFHhg54GHGhh45GHGH.PHASE_WINDOW_CREATE, 0)
		ui.Window.__del__(self)
...
#			self.Listdesjex.append(getChild("name_shamandes"))
	...
	def __SelectShape(self, shape):
		...
		self.shapeTextLine.SetText(SHAPE_NAME[shape])
	...
	def __CheckCreateCharacter(self, name):
		...
		if GFHhg54GHGhh45GHGH.IsInsultIn(name):
	...
	def OnUpdate(self):
		...
		event.UpdateEventSet(self.descIndex, xposEventSet+24, -(yposEventSet+16))
		...
		for i in xrange(4):
			self.curNameAlpha[i] += (self.destNameAlpha[i] - self.curNameAlpha[i]) / 10.0
			self.Listimgjex[i].SetAlpha(self.curNameAlpha[i])
			
		for i in xrange(4):
			self.curNameAlpha[i] += (self.destNameAlpha[i] - self.curNameAlpha[i]) / 10.0
#			self.Listdesjex[i].SetAlpha(self.curNameAlpha[i])
		...
		if -1 != self.reservingRaceIndex:
			if app.GetTime() - self.reservingStartTime >= 1.5:
				...
				GFHhg54GHGhh45GHGH.SendCreateCharacterPacket(chrSlot, textName, raceIndex, shapeIndex, statCon, statInt, statStr, statDex)
