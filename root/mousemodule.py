...
class CMouseController(object):
	...
	def ChangeCursor(self, cursorNum):
		try:
			...
			try:
				...
				self.curCursorImage = CursorImage("D:/Ymir Work/UI/Cursor/cursor.sub")
				
			try:
				
				(self.cursorPosX, self.cursorPosY) = self.cursorPosDict[cursorNum]
			except:
				(self.cursorPosX, self.cursorPosY) = (0, 0)
	...
	def AttachObject(self, Owner, Type, SlotNumber, ItemIndex, count = 0):
		...
		try:
			...
			if Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_INVENTORY or\
				Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_PRIVATE_SHOP or\
				Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SHOP or\
				Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SAFEBOX or\
				Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_MALL or\
				Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_DRAGON_SOUL_INVENTORY:
			...
			elif Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_EMOTION:
				image = fgGHGjjFHJghjfFG1545gGG.GetEmotionIconImage(ItemIndex)
				self.AttachedIconHandle = grpImage.GenerateFromHandle(image)

			elif Type == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_QUICK_SLOT:
				(quickSlotType, position) = fgGHGjjFHJghjfFG1545gGG.GetGlobalQuickSlot(SlotNumber)

				if quickSlotType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_INVENTORY:
				...
				elif quickSlotType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SKILL:
				...
				elif quickSlotType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_EMOTION:
					image = fgGHGjjFHJghjfFG1545gGG.GetEmotionIconImage(position)
					self.AttachedIconHandle = grpImage.GenerateFromHandle(image)
	...
	def IsAttachedMoney(self):
		if True == self.isAttached():
			if fgGHGjjFHJghjfFG1545gGG.ITEM_MONEY == self.GetAttachedItemIndex():
				return True
	...
	def AttachMoney(self, owner, type, count):
		...
		self.AttachedItemIndex = fgGHGjjFHJghjfFG1545gGG.ITEM_MONEY
		self.AttachedCount = count
	...
	def DeattachObject(self):
		...
		if self.AttachedIconHandle != 0:
			...
			if self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_INVENTORY or\
				self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_PRIVATE_SHOP or\
				self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SHOP or\
				self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SAFEBOX or\
				self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_MALL:

				item.DeleteIconInstance(self.AttachedIconHandle)

			elif self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SKILL:
				skill.DeleteIconInstance(self.AttachedIconHandle)

			elif self.AttachedType == fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_EMOTION:
	...
	def GetAttachedType(self):
		...
		if False == self.isAttached():
			return fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_NONE
