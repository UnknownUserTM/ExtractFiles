...
class PartyMemberInfoBoard(ui.ScriptWindow):
	...
	MEMBER_BUTTON_IMAGE_FILE_NAME_DICT = {	fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_LEADER : "party_state_leader",
											fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_ATTACKER : "party_state_attacker",
											fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_BERSERKER : "party_state_berserker",
											fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_TANKER : "party_state_tanker",
											fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_DEFENDER : "party_state_defender",
											fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_BUFFER : "party_state_buffer",
											fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_SKILL_MASTER : "party_state_skill_master",
											MEMBER_BUTTON_NORMAL : "party_state_normal",
											MEMBER_BUTTON_WARP : "party_skill_warp",
											MEMBER_BUTTON_EXPEL : "party_expel", }

	STATE_NAME_DICT =	{	fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_ATTACKER : localeInfo.PARTY_SET_ATTACKER,
							fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_BERSERKER : localeInfo.PARTY_SET_BERSERKER,
							fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_TANKER : localeInfo.PARTY_SET_TANKER,
							fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_DEFENDER : localeInfo.PARTY_SET_DEFENDER,
							fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_BUFFER : localeInfo.PARTY_SET_BUFFER,
							fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_SKILL_MASTER : localeInfo.PARTY_SET_SKILL_MASTER, }
	...
	def __CreateStateButton(self):
		for key, name in self.MEMBER_BUTTON_IMAGE_FILE_NAME_DICT.items():
			if key == fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_LEADER:
				continue
			button = ui.Button()
	...
	def __GetPartySkillLevel(self):
		slotIndex = fgGHGjjFHJghjfFG1545gGG.GetSkillSlotIndex(fgGHGjjFHJghjfFG1545gGG.SKILL_INDEX_TONGSOL)
	...
	def __ShowStateButton(self):
		...
		## Tanker
		if skillLevel >= 10:
			self.__AppendStateButton(xPos, y, fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_ATTACKER)
			xPos += 23

		## Attacker
		if skillLevel >= 20:
			self.__AppendStateButton(xPos, y, fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_BERSERKER)
			xPos += 23

		## Tanker
		if skillLevel >= 20:
			self.__AppendStateButton(xPos, y, fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_TANKER)
			xPos += 23

		## Buffer
		if skillLevel >= 25:
			self.__AppendStateButton(xPos, y, fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_BUFFER)
			xPos += 23

		## Skill Master
		if skillLevel >= 35:
			self.__AppendStateButton(xPos, y, fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_SKILL_MASTER)
			xPos += 23

		## Defender
		if skillLevel >= 40:
			self.__AppendStateButton(xPos, y, fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_DEFENDER)
			xPos += 23
	...
	def OnSelectState(self, state):
		...
		if state <= 0:
			GFHhg54GHGhh45GHGH.SendPartySetStatePacket(self.pid, self.state, False)

		else:

			if self.state <= 0:
				GFHhg54GHGhh45GHGH.SendPartySetStatePacket(self.pid, state, True)
	...
	def OnWarp(self):
		...
		if self.vid:
			GFHhg54GHGhh45GHGH.SendPartyUseSkillPacket(self.PARTY_SKILL_WARP, self.vid)
	...
	def OnExpel(self):
		...
		if not self.pid:
			return
		GFHhg54GHGhh45GHGH.SendPartyRemovePacket(self.pid)

	def OnMouseLeftButtonDown(self):

		if self.vid:
			fgGHGjjFHJghjfFG1545gGG.SetTarget(self.vid)
			fgGHGjjFHJghjfFG1545gGG.OpenCharacterMenu(self.vid)

			if mouseModule.mouseController.isAttached():
				attachedSlotPos = mouseModule.mouseController.GetAttachedSlotNumber()
				GFHhg54GHGhh45GHGH.SendExchangeStartPacket(self.vid)
				GFHhg54GHGhh45GHGH.SendExchangeItemAddPacket(attachedSlotPos, 0)
				mouseModule.mouseController.DeattachObject()
				return

		if fgGHGjjFHJghjfFG1545gGG.IsPartyLeader(fgGHGjjFHJghjfFG1545gGG.GetMainCharacterIndex()):
			if fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_LEADER != self.state:
...
class PartyMenu(ui.ThinBoard):
	...
	def __CreateModeButtons(self):
		...
		level.Show()
		self.modeButtonList[fgGHGjjFHJghjfFG1545gGG.PARTY_EXP_NON_DISTRIBUTION] = level
		...
		parity.Show()
		self.modeButtonList[fgGHGjjFHJghjfFG1545gGG.PARTY_EXP_DISTRIBUTION_PARITY] = parity
	...
	def __CreateButtons(self):
		...
		self.buttonDict[localeInfo.PARTY_BREAK_UP].SetEvent(GFHhg54GHGhh45GHGH.SendPartyExitPacket)
		...
		self.buttonDict[localeInfo.PARTY_LEAVE].SetEvent(GFHhg54GHGhh45GHGH.SendPartyExitPacket)
	...
	def OnPartyUseSkill(self):
		GFHhg54GHGhh45GHGH.SendPartyUseSkillPacket(PartyMemberInfoBoard.PARTY_SKILL_HEAL, 0)
	...
	def OnClickEXPLevel(self):
		...
		if self.isLeader:
			GFHhg54GHGhh45GHGH.SendPartyParameterPacket(fgGHGjjFHJghjfFG1545gGG.PARTY_EXP_NON_DISTRIBUTION)

	def OnClickEXPDistributeParity(self):
		self.__SetModeButton(self.distributionMode)
		if self.isLeader:
			GFHhg54GHGhh45GHGH.SendPartyParameterPacket(fgGHGjjFHJghjfFG1545gGG.PARTY_EXP_DISTRIBUTION_PARITY)
...
class PartyWindow(ui.Window):
	...
	def RemovePartyMember(self, pid):
		...
		if None != vid and fgGHGjjFHJghjfFG1545gGG.IsMainCharacterIndex(vid):

			self.ExitParty()
			fgGHGjjFHJghjfFG1545gGG.ExitParty()

		else:

			board.Destroy()
			fgGHGjjFHJghjfFG1545gGG.RemovePartyMember(pid)
	...
	def UpdatePartyMemberInfo(self, pid):
		...
		state = fgGHGjjFHJghjfFG1545gGG.GetPartyMemberState(pid)
		hpPercentage = fgGHGjjFHJghjfFG1545gGG.GetPartyMemberHPPercentage(pid)
		affectsList = fgGHGjjFHJghjfFG1545gGG.GetPartyMemberAffects(pid)
		...
		if None != vid:
			if fgGHGjjFHJghjfFG1545gGG.IsMainCharacterIndex(vid):
				if fgGHGjjFHJghjfFG1545gGG.PARTY_STATE_LEADER == fgGHGjjFHJghjfFG1545gGG.GetPartyMemberState(pid):
