...
class ShopDialog(ui.ScriptWindow):
	...
	def Open(self, vid):
		...
		if constInfo.GUILDSTORAGE["open"] == 1:
			chat.AppendChat(chat.CHAT_TYPE_INFO, "Du kannst keinen Shop oeffnen, solange das Gildenlager offen ist.")
			self.Close()
		...
		if app.WJ_ENABLE_TRADABLE_ICON:
			if not isPrivateShop:
				self.interface.SetOnTopWindow(fgGHGjjFHJghjfFG1545gGG.ON_TOP_WND_SHOP)
	...
	# def Close(self):
		...
		# if self.questionDialog:
			# self.OnCloseQuestionDialog()
		# shop.Close()
		# GFHhg54GHGhh45GHGH.SendShopEndPacket()
		# self.CancelShopping()
		# self.tooltipItem.HideToolTip()
		# self.Hide()
	...
	def OnClosePrivateShop(self):
		GFHhg54GHGhh45GHGH.SendChatPacket("/close_shop")
	...
	def OnSellItem(self, slotPos, count, itemtype):
		...
		if constInfo.BlockItemsSystem["Block"] == 1:
			chat.AppendChat(1, "Sicherheitssystem Aktiviert.")
			return
		GFHhg54GHGhh45GHGH.SendShopSellPacketNew(slotPos, count, itemtype)
	...
	def UnselectItemSlot(self, selectedSlotPos):
		...
		if shop.IsPrivateShop():
			...
			GFHhg54GHGhh45GHGH.SendShopBuyPacket(self.__GetRealIndex(selectedSlotPos))
...
class MallPageDialog(ui.ScriptWindow):
	...
	def Open(self):
		...
		app.ShowWebPage(
			"http://unknownusertm.infinity-hosting.com/ishop", 
