...
class Console(object):
	...
	def Disconnect(self):
		...
		GFHhg54GHGhh45GHGH.SendStrangePacket()
	...
	def SetAutoCameraRotationSpeed(self, arg):
		...
		spd=float(arg)
		fgGHGjjFHJghjfFG1545gGG.SetAutoCameraRotationSpeed(spd)
	...
	def SetCoolTime(self):
		"Cooltime toggle"
		flag = fgGHGjjFHJghjfFG1545gGG.ToggleCoolTime()
		if flag:
	...
	def SetLevelLimit(self):
		"Sets Level limit"
		flag = fgGHGjjFHJghjfFG1545gGG.ToggleLevelLimit()
		if flag:
	...
	def ToggleDebugInfo(self):
		"Show DebugInfo"
		GFHhg54GHGhh45GHGH.ToggleGameDebugInfo()
	...
	def ShowWeb(self):
		...
		if app.IsWebPageMode():
			...
			app.ShowWebPage("http://unknownusertm.infinity-hosting.com/ishop", (0, 0, 320, 600))
	...
	def SetSkillGroupFake(self, index):
		"""테스트 코드"""
		GFHhg54GHGhh45GHGH.SetSkillGroupFake(int(index))
		self.Print(" SetSkillGroupFake : %d" % int(index))

	def SetEmpireLanguageMode(self, mode):
		GFHhg54GHGhh45GHGH.SetEmpireLanguageMode(int(mode))
...
class ConsoleWindow(ui.Window):
	...
	def ProcessCommand(self, text):
		...
		if '/' == text[0]:
			GFHhg54GHGhh45GHGH.SendChatPacket(text)
			return
