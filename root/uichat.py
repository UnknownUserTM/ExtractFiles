	...
	def OnIMEReturn(self):
		...
		if (textLen > 0) and (textLen != textSpaceCount):
			if '#' == text[0]:
				...
				self.__SendShoutChatPacket(str("||H"+str(fgGHGjjFHJghjfFG1545gGG.GetMainCharacterName())+":13|h[PN]|h|r ")+text[1:len(text)])
			else:
...
class ChatLogWindow(ui.Window):
	...
	BLOCK_WIDTH = 32
	CHAT_MODE_NAME = ( localeInfo.CHAT_NORMAL, localeInfo.CHAT_PARTY, localeInfo.CHAT_GUILD, localeInfo.CHAT_SHOUT, localeInfo.CHAT_INFORMATION, localeInfo.CHAT_NOTICE, localeInfo.CHAT_DEBUG, )
