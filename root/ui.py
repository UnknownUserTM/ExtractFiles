...
class ToolTipButton(Button):
	def __init__(self):
		...
		self.heightBoard = 20
		self.buttonWidth = 0
		self.toolTipBox = ThinBoard()
		self.toolTipBox.SetParent(self)
		self.toolTipBox.SetPosition(self.GetWidth()/2,10)
		self.toolTipBox.SetSize(self.widthBoard,self.heightBoard)
		self.toolTipBox.SetWindowHorizontalAlignCenter()
		# self.toolTipBox.SetWindowVerticalAlignCenter()
		self.toolTipBox.Hide()
		
		self.textLine = TextLine()
		self.textLine.SetParent(self.toolTipBox)
		self.textLine.SetPosition(50,8)
		self.textLine.SetHorizontalAlignCenter()
		# self.textLine.SetText("TestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTestTest")
		# self.textLine.SetLimitWidth(45)
		# self.textLine.SetMultiLine()
		self.textLine.SetOutline()
		self.textLine.Show()
		# self.textLine.SetText("Und was ist wenn ich hier nun einen sehr viel langeren satz reinschreib")
		# self.textLine.SetText(str(self.textLine.GetTextSize()))
		self.ArrangeToolTip()
	...
	def AppendToolTipTextLine(self,text):
		self.textLine.SetText(text)
		
	# def ClearToolTip(self):
		
	def ArrangeToolTip(self):
		(textwidth, textheight) = self.textLine.GetTextSize()
		
		self.widthBoard = max(80,textwidth+32)
		self.heightBoard = max(20,textheight+16)
		
		self.toolTipBox.SetSize(self.widthBoard,self.heightBoard)
		self.toolTipBox.SetWindowHorizontalAlignCenter()
		self.toolTipBox.SetPosition((self.buttonWidth / 2),0-(self.heightBoard+8))
		self.textLine.SetPosition(self.widthBoard/2,8)
		
	def ShowToolTip(self):
		self.toolTipBox.Show()
...
####Item level text/icon END ###
	...
	def SetEmotionSlot(self, renderingSlotNumber, emotionIndex):
		...
		icon = fgGHGjjFHJghjfFG1545gGG.GetEmotionIconImage(emotionIndex)
...
class RoofBar(Window):
	...
	def MakeTitleBar(self, width, color):
		...
		if localeInfo.IsARABIC():
			imgLeft.LoadImage("/titlebar_left.tga")
			imgCenter.LoadImage("/titlebar_center.tga")
			imgRight.LoadImage("/titlebar_right.tga")
		else:
			imgLeft.LoadImage("yamato_roof/left.tga")
			imgCenter.LoadImage("yamato_roof/center.tga")
			imgRight.LoadImage("yamato_roof/right.tga")
		...
		btnClose.SetUpVisual("yamato_button/close_n.dds")
		btnClose.SetOverVisual("yamato_button/close_h.dds")
		btnClose.SetDownVisual("yamato_button/close_p.dds")
	...
	def SetWidth(self, width):
		...
		self.imgCenter.SetPosition(self.BLOCK_WIDTH, -2)
		self.imgRight.SetPosition(width - self.BLOCK_WIDTH, 0)

		if localeInfo.IsARABIC():
			self.btnClose.SetPosition(3, 3)
		else:
			self.btnClose.SetPosition(width - self.btnClose.GetWidth() - 9, 10)
...
class Board(Window):
	...
	def __init__(self):
		...
		self.MakeBoard("yamato_board/Board_Corner_", "yamato_board/Board_Line_")
		self.MakeBase()
	...
	def MakeBase(self):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage("yamato_board/Board_Base.tga")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
...
class BoardWithRoofBar(Window):
	...
	def __init__(self):
		...
		roofBar = RoofBar()
		roofBar.SetParent(self)
		roofBar.MakeTitleBar(0,"red")
		roofBar.SetPosition(-8,7)
		roofBar.Show()
		
		baseBoard = Board()
		baseBoard.SetParent(self)
		baseBoard.SetPosition(0,50)
		baseBoard.Show()

		self.roofBar = roofBar
	...
	def SetSize(self, width, height):
		Window.SetSize(self,width+self.ADD_WIDTH,height+self.ADD_HEIGHT)
		self.roofBar.SetWidth(width + 45)
		self.baseBoard.SetSize(width+30, height+30)
		
	def SetCloseEvent(self, event):
		self.roofBar.SetCloseEvent(event)
...
class QuestListBox(Window):
	...
	def InsertTitleItem(self, text):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
	...
	def InsertObjectiveItem(self, text, status):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
		
		self.objectiveList[len(self.objectiveList)] = len(self.itemList)
		...
		if status:
			textLine.SetText("        " + "|Eemoji/objective_d|e " + text)
		else:
			textLine.SetText("        " + "|Eemoji/objective_o|e " + text)
		# textLine.SetOutline()
		textLine.Show()
	...
	def ChangeObjectiveItem(self,index,text,status):
		index = self.objectiveList[index]
		if status:
			text = "        " + "|Eemoji/objective_d|e " + text
		else:
			text = "        " + "|Eemoji/objective_o|e " + text	
		
		self.textDict[index] = text
		self.itemList[index].SetText(text)
		
		
	def InsertCurrencyItem(self, text, currency):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
		...
		textLine.SetText("      " + "|Eemoji/" + str(currency) + "|e " + str(text))
	...
	def InsertItemRewardItem(self, itemVnum, itemCount):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		item.SelectItem(itemVnum)
		text = "      " + "|Eemoji/item|e " + str(itemCount) + "x " + str(item.GetItemName())
		self.textDict[len(self.itemList)] = text
		textLine = TextLine()
		textLine.SetParent(self)
		textLine.SetOutline()
	...
	def InsertAttributeItem(self,text):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
		...
		textLine.SetOutline()
		textLine.SetFontName(localeInfo.UI_DEF_FONT_LARGE)
		textLine.SetText("      " + "|Eemoji/bonus|e " + text)
	...
	def InsertEmptyItem(self):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = ""
	...
	def InsertQuestViewTitle(self, text):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
		...
		textLine.SetText("    " + "|Eemoji/quest_new|e  " + text)
		textLine.SetFontColor(1.0, 0.7843, 0.0)
		textLine.SetOutline()
		textLine.Show()
...
class QuestViewListBox(Window):
	...
	def InsertQuestViewTitle(self, text):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
		self.questIndex[len(self.questIndex)] = len(self.itemList)
		textLine = TextLine()
		textLine.SetParent(self)
	...
	def InsertObjectiveItem(self, text, status):
		...
		self.keyDict[len(self.itemList)] = index
		self.textDict[len(self.itemList)] = text
		self.questTimer[len(self.itemList)] = 0
		textLine = TextLine()
		textLine.SetParent(self)
		textLine.SetOutline()
	
		if status == self.QUEST_STATUS_NOT_STARTED:
			textLine.SetText("        " + "|Eemoji/objective_o|e " + text)
			textLine.SetFontColor(0.9, 0.4745, 0.4627)
		elif status == self.QUEST_STATUS_IN_PROGRESS:
			textLine.SetText("        " + "|Eemoji/objective_o|e " + text)
			textLine.SetFontColor(0.6911, 0.8754, 0.7068)
		else:
			textLine.SetText("        " + "|Eemoji/objective_d|e " + text)
			textLine.SetFontColor(0.5411, 0.7254, 0.5568)
		textLine.SetOutline()
		textLine.Show()
	...
	def InsertObjectiveItem_OLD(self, text, status):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = text
	...
	def InsertObjectiveItemTimer(self, qid, timer):
		self.keyDict[len(self.itemList)] = len(self.itemList)
		self.textDict[len(self.itemList)] = ""
		
		self.objectiveList[len(self.objectiveList)] = len(self.itemList)
		self.questIndex[len(self.questIndex)] = len(self.itemList)
		self.questTimer[len(self.itemList)] = app.GetTime() + timer
		
		textLine = TextLine()
		textLine.SetParent(self)
		textLine.SetOutline()

		# textLine.SetText("        " + "|Eemoji/objective_o|e " )
	...
	def ChangeObjectiveItem(self,index,text,status):
		...
		if status == self.QUEST_STATUS_NOT_STARTED:
			text = "        " + "|Eemoji/objective_o|e " + text
			self.itemList[index].SetFontColor(0.9, 0.4745, 0.4627)
		elif status == self.QUEST_STATUS_IN_PROGRESS:
			text = "        " + "|Eemoji/objective_o|e " + text
			self.itemList[index].SetFontColor(0.6911, 0.8754, 0.7068)
		else:
			text = "        " + "|Eemoji/objective_d|e " + text
			self.itemList[index].SetFontColor(0.5411, 0.7254, 0.5568)
	...
	def OnUpdate(self):
		for i in xrange(len(self.objectiveList)):
			index = self.objectiveList[i]
			if self.questTimer[index] > 0:
				timeLeft = self.questTimer[index] - app.GetTime()
				if timeLeft <= 0:
					text = "        " + "|Eemoji/objective_d|e Abgelaufen!"
				else:
					text = "        " + "|Eemoji/objective_d|e " + str(timeLeft)
...
class PythonScriptLoader(object):
	...
	def LoadScriptFile(self, window, FileName):
		...
		try:
			...
			self.ScriptDictionary["DRAGON_SOUL_EQUIPMENT_SLOT_START"] = fgGHGjjFHJghjfFG1545gGG.DRAGON_SOUL_EQUIPMENT_SLOT_START
	...
	def LoadChildren(self, parent, dicChildren):
		...
		for ElementValue in ChildrenList:
			...
			if Type == "window":
				...
				parent.Children[Index] = RoofBar()
				parent.Children[Index].SetParent(parent)
				self.LoadElementRoofBar(parent.Children[Index], ElementValue, parent)
				...
				parent.Children[Index].SetParent(parent)
				self.LoadElementBoardWithRoofBar(parent.Children[Index], ElementValue, parent)				
			...
			elif Type == "questlistbox":
				parent.Children[Index] = QuestListBox()
				parent.Children[Index].SetParent(parent)
				self.LoadElementListBox(parent.Children[Index], ElementValue, parent)			
	...
	def LoadElementBoardWithRoofBar(self, window, value, parentWindow):
...
class AnnouncementCheckBoxLanguage(ImageBox):
	def __init__(self, parent, x, y, event):
...
class ChatFilterCheckBoxLanguage(ImageBox):
	def __init__(self, parent, bLanguage, x, y, event):
		...
		self.AttributeLocked = (app.GetLanguageIndex(app.GetLanguage()) == bLanguage)
		if (self.AttributeLocked):
			wndSlot = Bar()
			wndSlot.SetParent(self)
			wndSlot.SetSize(self.GetWidth() - 4, self.GetHeight() - 4)
			wndSlot.SetColor(0xFF008116)
			wndSlot.SetPosition(2, 2)
			wndSlot.Show()
			self.wndSlot = wndSlot
		...
		image.SetWindowVerticalAlignCenter()
		image.Hide()
		self.image, self.bLanguage, self.state, self.event = image, bLanguage, app.LANGUAGE_UNSELECTED, event
	...
	def IsChecked(self):
		return self.state == app.LANGUAGE_SELECTED

	def UnSelect(self):
		if (not self.AttributeLocked):
			self.image.Hide()
			self.state = app.LANGUAGE_UNSELECTED

	def Select(self):
		self.image.Show()
		self.state = app.LANGUAGE_SELECTED
