...
#				pyScrLoader.LoadScriptFile(self, "uiscript/passworddialog.py")
	...
	def OnAccept(self):
		GFHhg54GHGhh45GHGH.SendChatPacket(self.sendMessage + self.passwordValue.GetText())
		self.CloseDialog()
		return True
...
class ChangePasswordDialog(ui.ScriptWindow):
	...
	def OnAccept(self):
		...
		if newPasswordText != newPasswordCheckText:
			...
			return True
		GFHhg54GHGhh45GHGH.SendChatPacket("/safebox_change_password %s %s" % (oldPasswordText, newPasswordText))
		self.Close()
		return True
...
class SafeboxWindow(ui.ScriptWindow):
	...
	def OnPickMoney(self, money):
		mouseModule.mouseController.AttachMoney(self, fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SAFEBOX, money)
...
class MallWindow(ui.ScriptWindow):
	...
	def Close(self):
		GFHhg54GHGhh45GHGH.SendChatPacket("/mall_close")
