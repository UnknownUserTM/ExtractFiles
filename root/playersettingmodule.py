...
def RegisterSkill(race, group, empire=0):
	...
	if SKILL_INDEX_DICT.has_key(job):
		...
		if SKILL_INDEX_DICT[job].has_key(group):
			...
			for i in xrange(len(activeSkillList)):
				...
				if i != 6 and i != 7:
					fgGHGjjFHJghjfFG1545gGG.SetSkill(i+1, skillIndex)

			supportSkillList = SKILL_INDEX_DICT[job]["SUPPORT"]

			for i in xrange(len(supportSkillList)):
				fgGHGjjFHJghjfFG1545gGG.SetSkill(i+100+1, supportSkillList[i])
	...
	if 0 != empire:
		...
		for i in xrange(3):
			if (i+1) != empire:
				languageSkillList.append(fgGHGjjFHJghjfFG1545gGG.SKILL_INDEX_LANGUAGE1+i)
		for i in xrange(len(languageSkillList)):
			fgGHGjjFHJghjfFG1545gGG.SetSkill(107+i, languageSkillList[i])

	## Guild Skill
	for i in xrange(len(PASSIVE_GUILD_SKILL_INDEX_LIST)):
		fgGHGjjFHJghjfFG1545gGG.SetSkill(200+i, PASSIVE_GUILD_SKILL_INDEX_LIST[i])

	for i in xrange(len(ACTIVE_GUILD_SKILL_INDEX_LIST)):
		fgGHGjjFHJghjfFG1545gGG.SetSkill(210+i, ACTIVE_GUILD_SKILL_INDEX_LIST[i])
...
def RegisterSkillAt(race, group, pos, num):
	...
	fgGHGjjFHJghjfFG1545gGG.SetSkill(pos+1, num)
...
def __InitData():
	...
	fgGHGjjFHJghjfFG1545gGG.RegisterCacheEffect(fgGHGjjFHJghjfFG1545gGG.EFFECT_PICK, "d:/ymir work/effect/etc/click/click.mse")
...
def __LoadGameEffect():
	...
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+41, "Bip01", "locale/de/effect/fgGHGjjFHJghjfFG1545gGG.mse")
	#chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+42, "Bip01", "d:/ymir work/effect/etc/ranking_battle/ranker_battle_01.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+42, "Bip01", "d:/ymir work/effect/etc/direction/direction_land_dragonroom.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+43, "Bip01", "d:/ymir work/effect/etc/direction/direction_land_dragonroom.mse")
	#chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+44, "Bip01", "d:/ymir work/effect/etc/direction/direction_land_dragonroom.mse")
	
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+45, "Bip01", "locale/de/effect/serverleitung.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+46, "Bip01", "locale/de/effect/teamleitung.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+47, "Bip01", "locale/de/effect/super_game_master.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+48, "Bip01", "locale/de/effect/entwicklung_technik.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+49, "Bip01", "locale/de/effect/server_support.mse")
	chrmgr.RegisterEffect(chrmgr.EFFECT_AFFECT+50, "Bip01", "locale/de/effect/serverentwicklung.mse")
	...
	GFHhg54GHGhh45GHGH.RegisterEmoticonString("(fish)")
	...
	chrmgr.RegisterEffect(chrmgr.EFFECT_EMOTICON+13, "", "d:/ymir work/effect/etc/buff/buff_symbol_heldenmedaille.mse")
	GFHhg54GHGhh45GHGH.RegisterEmoticonString("(effect_item_buff_hero_medal)")
	
	chrmgr.RegisterEffect(chrmgr.EFFECT_EMOTICON+14, "", "d:/ymir work/effect/etc/buff/buff_item_happy_ring.mse")
	GFHhg54GHGhh45GHGH.RegisterEmoticonString("(effect_item_buff_happiness_ring)")	

	chrmgr.RegisterEffect(chrmgr.EFFECT_EMOTICON+15, "", "d:/ymir work/effect/etc/buff/buff_item_deadly_power.mse")
	GFHhg54GHGhh45GHGH.RegisterEmoticonString("(effect_item_buff_deadly_power)")	
