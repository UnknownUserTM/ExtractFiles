...
class DeclareGuildWarDialog(ui.ScriptWindow):
	...
	def __OnOK(self):
		...
		GFHhg54GHGhh45GHGH.SendChatPacket("/war %s %d" % (text, type))
		self.Close()
...
class GuildWindow(ui.ScriptWindow):
	...
	def Open(self):
		...
		guildID = GFHhg54GHGhh45GHGH.GetGuildID()
	...
	def __OnSelectMark(self, markFileName):
		ret = GFHhg54GHGhh45GHGH.UploadMark("upload/"+markFileName)

		# MARK_BUG_FIX
		if GFHhg54GHGhh45GHGH.ERROR_MARK_UPLOAD_NEED_RECONNECT == ret:
	...
	def __OnSelectSymbol(self, symbolFileName):
		GFHhg54GHGhh45GHGH.UploadSymbol("upload/"+symbolFileName)
	...
	def __OnClickDepositButton(self):
		...
		moneyDialog.Open(fgGHGjjFHJghjfFG1545gGG.GetMoney())
	...
	def OnDeposit(self, money):
		GFHhg54GHGhh45GHGH.SendGuildDepositMoneyPacket(money)

	def OnWithdraw(self, money):
		GFHhg54GHGhh45GHGH.SendGuildWithdrawMoneyPacket(money)

	def OnOffer(self, exp):
		GFHhg54GHGhh45GHGH.SendGuildOfferPacket(exp)
	...
	def OnPostComment(self):
		...
		GFHhg54GHGhh45GHGH.SendGuildPostCommentPacket(text[:50])
	...
	def OnDeleteComment(self, index):
		...
		GFHhg54GHGhh45GHGH.SendGuildDeleteCommentPacket(commentID)

	def OnRefreshComments(self):
		GFHhg54GHGhh45GHGH.SendGuildRefreshCommentsPacket(0)
	...
	def OnChangeMemberGrade(self, lineIndex, gradeNumber):
		...
		GFHhg54GHGhh45GHGH.SendGuildChangeMemberGradePacket(PID, gradeNumber)
	...
	def OnEnableGeneral(self, lineIndex):
		...
		GFHhg54GHGhh45GHGH.SendGuildChangeMemberGeneralPacket(pid, 1 - general)
	...
	def OnChangeGradeName(self):
		...
		GFHhg54GHGhh45GHGH.SendGuildChangeGradeNamePacket(gradeNumber, gradeName)
		return True

	def OnCheckAuthority(self, argIndex, argAuthority):
		name, authority = guild.GetGradeData(argIndex)
		GFHhg54GHGhh45GHGH.SendGuildChangeGradeAuthorityPacket(argIndex, authority ^ argAuthority)
	...
	def __OnHealGSP(self):
		GFHhg54GHGhh45GHGH.SendGuildChargeGSPPacket(self.questionDialog.healGSP)
	...
	def OnPickUpGuildSkill(self, skillSlotIndex, type):
		...
		if False == mouseController.isAttached():
			...
			skillIndex = fgGHGjjFHJghjfFG1545gGG.GetSkillIndex(skillSlotIndex)
			...
			if skill.CanUseSkill(skillIndex) and skillLevel > 0:
				...
				if app.IsPressed(app.DIK_LCONTROL):
					...
					fgGHGjjFHJghjfFG1545gGG.RequestAddToEmptyLocalQuickSlot(fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SKILL, skillSlotIndex)
					return

				mouseController.AttachObject(self, fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_SKILL, skillSlotIndex, skillIndex)

		else:
			mouseController.DeattachObject()

	def OnUseGuildSkill(self, slotNumber, type):
		skillIndex = fgGHGjjFHJghjfFG1545gGG.GetSkillIndex(slotNumber)
		skillLevel = guild.GetSkillLevel(slotNumber)

		if skillLevel <= 0:
			return

		fgGHGjjFHJghjfFG1545gGG.UseGuildSkill(slotNumber)

	def OnUpGuildSkill(self, slotNumber, type):
		skillIndex = fgGHGjjFHJghjfFG1545gGG.GetSkillIndex(slotNumber)
		GFHhg54GHGhh45GHGH.SendChatPacket("/gskillup " + str(skillIndex))
...
class BuildGuildBuildingWindow(ui.ScriptWindow):
	...
	def Build(self):
		...
		if "BUILDIN" == self.type:
			for i in xrange(len(self.raceList)):
				...
				rot = self.rotList[i]
				GFHhg54GHGhh45GHGH.SendChatPacket("/build c %d %d %d %d %d %d" % (race, int(self.x+xPos), int(self.y+yPos), self.rot_x, self.rot_y, rot))
		else:
	...
	def __IsEnoughMoney(self):
		...
		curMoney = fgGHGjjFHJghjfFG1545gGG.GetMoney()
	...
	def __IsEnoughMaterialStone(self):
		...
		curStoneCount = fgGHGjjFHJghjfFG1545gGG.GetItemCountByVnum(MATERIAL_STONE_ID)
	...
	def __IsEnoughMaterialLog(self):
		...
		curLogCount = fgGHGjjFHJghjfFG1545gGG.GetItemCountByVnum(MATERIAL_LOG_ID)
	...
	def __IsEnoughMaterialPlywood(self):
		...
		curPlywoodCount = fgGHGjjFHJghjfFG1545gGG.GetItemCountByVnum(MATERIAL_PLYWOOD_ID)
	...
	def SettleCurrentPosition(self):
		...
		import debugInfo
		if debugInfo.IsDebugMode():
			guildID = fgGHGjjFHJghjfFG1545gGG.GetGuildID()

		if guildID != fgGHGjjFHJghjfFG1545gGG.GetGuildID():
			return
...
AcceptButton:
	BuildGuildBuildingWindow.Build:
		GFHhg54GHGhh45GHGH.SendChatPacket("/build c vnum x y x_rot y_rot z_rot")
...
건물 부수기:
	uiTarget.TargetBoard.__OnDestroyBuilding
		GFHhg54GHGhh45GHGH.SendChatPacket("/build d vid")
"""
...
if __name__ == "__main__":
	...
	class TestGame(ui.Window):
		def __init__(self):
			...
			fgGHGjjFHJghjfFG1545gGG.SetMainCharacterIndex(1)
