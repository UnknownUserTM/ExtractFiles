...
# class QuestionListBoxDialog(ui.ScriptWindow):
	...
	# def __CreateDialog(self):
		...
		# pyScrLoader.LoadScriptFile(self, "exscript/questionlistboxdialog.py")
...
class QuestionBattlezoneDialogLeave(ui.ScriptWindow):
	...
	def __CreateDialog(self):
		pyScrLoader = ui.PythonScriptLoader()
		pyScrLoader.LoadScriptFile(self, "uiscript/battlezoneleave.py")

		self.board = self.GetChild("board")
		self.textLine = self.GetChild("points")
		self.normalLeave = self.GetChild("normal")
		self.instantLeave = self.GetChild("instant")
		self.cancelButton = self.GetChild("cancel")
	...
	def SAFE_SetAcceptEvent(self, event):
		self.normalLeave.SAFE_SetEvent(event)

	def SAFE_SetCancelEvent(self, event):
		self.cancelButton.SAFE_SetEvent(event)

	def SetAcceptEvent(self, event):
		self.normalLeave.SetEvent(event)

	def SetDestroyEvent(self, event):
		self.instantLeave.SetEvent(event)
