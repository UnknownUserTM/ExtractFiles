...
def IsBuildingPrivateShop():
	global g_isBuildingPrivateShop
	if fgGHGjjFHJghjfFG1545gGG.IsOpenPrivateShop() or g_isBuildingPrivateShop:
		return True
	else:
		return False
...
class PrivateShopAdvertisementBoard(ui.ThinBoard):
	...
	def OnUpdate(self):
		...
		if systemSetting.IsShowSalesText():
			...
			for key in g_privateShopAdvertisementBoardDict.keys():
				if  fgGHGjjFHJghjfFG1545gGG.GetMainCharacterIndex() == key:  #상점풍선을 안보이게 감추는 경우에도, 플레이어 자신의 상점 풍선은 보이도록 함. by 김준호
					g_privateShopAdvertisementBoardDict[key].Show() 	
					x, y = chr.GetProjectPosition(fgGHGjjFHJghjfFG1545gGG.GetMainCharacterIndex(), 220)
...
class PrivateShopBuilder(ui.ScriptWindow):
	...
	def Open(self, title):
		...
		if app.WJ_ENABLE_TRADABLE_ICON:
			self.lockedItems = {i:(-1,-1) for i in range(shop.SHOP_SLOT_COUNT)}
			self.interface.SetOnTopWindow(fgGHGjjFHJghjfFG1545gGG.ON_TOP_WND_PRIVATE_SHOP)
	...
	def OnSelectEmptySlot(self, selectedSlotPos):
		...
		if isAttached:
			...
			if item.IsAntiFlag(item.ANTIFLAG_GIVE) or item.IsAntiFlag(item.ANTIFLAG_MYSHOP):
				...
				return
			if app.WJ_ENABLE_TRADABLE_ICON and fgGHGjjFHJghjfFG1545gGG.SLOT_TYPE_INVENTORY == attachedSlotType:
	...
	def CancelInputPrice(self):
		if app.WJ_ENABLE_TRADABLE_ICON:
			itemInvenPage = self.priceInputBoard.sourceSlotPos / fgGHGjjFHJghjfFG1545gGG.INVENTORY_PAGE_SIZE
			itemSlotPos = self.priceInputBoard.sourceSlotPos - (itemInvenPage * fgGHGjjFHJghjfFG1545gGG.INVENTORY_PAGE_SIZE)
